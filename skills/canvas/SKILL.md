---
name: canvas
description: Display HTML content on connected Clawdbot nodes (Mac app, iOS, Android). Use for games, visualizations, dashboards, and interactive demos.
...
```bash
cat ~/.liteclaw/liteclaw.json | jq '.gateway.bind'
```
...
**Debug steps:**
1. Check server bind: `cat ~/.liteclaw/liteclaw.json | jq '.gateway.bind'`
